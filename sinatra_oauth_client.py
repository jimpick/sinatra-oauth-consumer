...
import httplib # used for talking to the Sinatra OAuth server
...
def test_sinatraoauth():
    ...
    signature_method = oauth.OAuthSignatureMethod_HMAC_SHA1()
        # HMAC_SHA1 is Sinatra OAuth's preferred hashing method
    ...
    # use our fetch_response method to send the request to Sinatra OAuth
    resp=fetch_response(oauth_request, connection)
    print 'Sinatra OAuth response was: %s' % resp
    ...
    resp=fetch_response(oauth_request, connection) # use our fetch_response
        # method to send the request to Sinatra OAuth
    print 'Sinatra OAuth response was: %s' % resp
    ...
    oauth_request = oauth.OAuthRequest.from_consumer_and_token(consumer,
        http_method='POST', token=token, http_url=UPDATE_API_URL,
        parameters=params)
    oauth_request.sign_request(signature_method, consumer, token)
    ...
    # do the POST
    connection.request('POST', UPDATE_API_URL, oauth_request.to_postdata(),
        headers)
    print 'Sinatra OAuth says: %s' % connection.getresponse().read()
        # print the response
